...
namespace Maru.Routing {
	...
	public class RouteHandler : IEnumerable<RouteHandler> {
		...
		public RouteHandler (string pattern, string [] methods, IMaruTarget target) : this (new string [] { pattern }, methods, target)
		{
		}
		...
		public RouteHandler (string pattern, string method, IMaruTarget target) : this (new string [] { pattern }, new string [] { method }, target)
		{
		}
		...
		public RouteHandler (string [] patterns, string [] methods, IMaruTarget target)
		{
			Target = target;
		...
		public IMaruTarget Find (IHttpRequest request)
		{
			return Find (request, 0);
		}
		
		private IMaruTarget Find (IHttpRequest request, int uri_start)
		{
			...
			foreach (RouteHandler handler in Children) {
				IMaruTarget res = handler.Find (request, uri_start);
				if (res != null) {
					if (uri_data != null)
						request.UriData.Add (uri_data);
					return res;
